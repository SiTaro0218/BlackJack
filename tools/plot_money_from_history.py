import os, csv, matplotlib
import os
import csv
import matplotlib
import sys
matplotlib.use('Agg')
import matplotlib.pyplot as plt

BASE = os.path.normpath(os.path.join(os.path.dirname(__file__), '..'))
HIST = os.path.join(BASE, 'logs', 'test_top1.history.csv')
OUT = os.path.join(BASE, 'figures', 'test_top1_money.png')

# ensure project base is importable
sys.path.insert(0, BASE)
from config import INITIAL_MONEY

if not os.path.exists(HIST):
    print('History file not found:', HIST)
    raise SystemExit(2)

# parse per-episode rewards
rewards = []
cur_sum = 0.0
with open(HIST, newline='', encoding='utf-8', errors='replace') as f:
    r = csv.DictReader(f)
    for row in r:
        try:
            rew = float(row.get('reward', 0) or 0)
        except:
            rew = 0.0
        cur_sum += rew
        res = (row.get('result') or '').strip().lower()
        if res and res not in ['', 'unsettled', 'retry']:
            rewards.append(cur_sum)
            cur_sum = 0.0
# leftover
if cur_sum != 0.0:
    rewards.append(cur_sum)

print('Parsed', len(rewards), 'episodes from', HIST)
start_money = INITIAL_MONEY
money = [start_money]
for r in rewards:
    money.append(money[-1] + r)

# simple stats
final_money = money[-1]
delta = final_money - start_money
print('Start money:', start_money)
print('Final money:', final_money)
print('Delta:', delta)

# plot
plt.figure(figsize=(10,5))
plt.plot(range(0, len(money)), money, marker='o', markersize=3)
plt.xlabel('Episode')
plt.ylabel('Money')
plt.title(f'Money progression (test_top1)  start {start_money} -> final {final_money} (Δ {delta})')
plt.grid(True)
plt.tight_layout()
plt.savefig(OUT)
print('Saved figure to', OUT)
